...
function musiclistandplay(folder, songs) {
    ...
    document.querySelector(".mar").innerHTML = decodeURIComponent(songs[0])
    ...
    for (const song of songs) {
        ...
        // console.log(song1);        
        ul.innerHTML += `<li>${decodeURIComponent(song1)}</li>`
        // console.log(song1);
...
async function displayalbum() {
    ...
    Array.from(document.getElementsByClassName("card")).forEach((e) => {
        ...
        //    console.log(`/${folder}/${songs[0]}`);    
            ...
            document.querySelector(".songplay").getElementsByTagName("img")[0].src="images/play2.svg";
...
//     for (let i = 0; i < anchorsarray.length; i++) {
//         let e = anchorsarray[i]; // Use `let` to ensure block-scoping
//         if (e.href.includes("/songs/")) {
//             let foldername = e.href.split("/").slice(-2)[0]; // Use `let` to ensure block-scoping
//             ar.push(foldername);        
//                 const a = await fetch(`http://127.0.0.1:3000/songs/${foldername}/info.json`);
...
//                         <div>${info.Description}</div>
//                     </div>`;
//                 document.querySelector(".music").innerHTML += album; // Ensure .music exists
...
// // Ensure the document is ready before executing `just`
// just()
...
async function main() {
    ...
    currentsong.addEventListener("timeupdate", () => {
        ...
        if(currentsong.currentTime==currentsong.duration & document.querySelector(".loop").src.includes("wholeloop.svg")){
            file = currentsong.src.split("/").slice(-1)
            index = songs.indexOf(file[0])
            if(index<songs.length-1){
                pat = `/${folder}/` + songs[index +1]
                document.querySelector(".mar").innerHTML = decodeURIComponent(songs[index + 1])
                playmusic(pat)
                document.querySelector(".songplay").innerHTML = `<img src="images/play2.svg" alt="">`
            }
            else{
        ...
        if(currentsong.currentTime==currentsong.duration & document.querySelector(".loop").src.includes("singleloop.svg")){
            currentsong.play()
            document.querySelector(".songplay").innerHTML = `<img src="images/play2.svg" alt="">`
        }
...
document.querySelector(".loop").addEventListener("click",(e) => {
    ...
    if(loop.src.includes("singleloop.svg")){
       loop.src="images/wholeloop.svg"
    }
    else if(loop.src.includes("wholeloop.svg")){
       loop.src="images/no-loop.svg"
    }
    else{
       loop.src="images/singleloop.svg"
    }
